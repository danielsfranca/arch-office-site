...
import { getClientDataAction, getClientDataByEmail } from "../actions/client-data";
...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      // 1. Try to load by Session Email (Google/Outlook/Auth)
      if (status === "authenticated" && session?.user?.email) {
        try {
          const data = await getClientDataByEmail(session.user.email);
          if (data) {
            setClientData(data);
            setFirstTime(false);
            setIsLoading(false);
      ...
      // 2. Fallback to LocalStorage (Manual Serial)
      const savedSerial = localStorage.getItem("clientSerial");
      if (savedSerial) {
        setSerialInput(savedSerial);
        try {
          const data = await getClientDataAction(savedSerial);
          if (data) {
            setClientData(data);
            setFirstTime(false);
          }
        } catch (error) {
          console.error("Error fetching by serial:", error);
        }
      }
      setIsLoading(false);
    }
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center bg-white"><div className="loader"></div><style jsx>{`.loader { width: 30px; height: 30px; border: 3px solid #eee; border-top-color: #1a1a1a; border-radius: 50%; animation: spin 1s linear infinite; } @keyframes spin { to { transform: rotate(360deg); } }`}</style></div>;
  }
        ...
        <div className="modal-content">
          ...
          {selectedService && (
            <div className="serial-input">
              <label>Vincular Projeto (Código de Acesso)</label>
              <p className="text-xs text-gray-400 mb-2">Não encontramos um projeto vinculado ao seu email <strong>{session?.user?.email}</strong>. Digite o código do projeto para vincular.</p>
